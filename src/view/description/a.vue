<template>
  <div id="table-dome">
    <page-layout>
      <page-header
        :title="title"
        describe="通过对能耗数据的分析，评估各种节能措施的效果，并不断优化节能方案，以实现更低的能源消耗和碳排放。"
      ></page-header>
    </page-layout>
    <page-layout>
      <a-input-search
        ...
        <a-col :span="24">
          <a-card v-for="(item, index) of data" :key="index">
            ...
            >
              <!-- <template #extra>
                <a-space>
                  <a-button type="primary" @click="$alert('编辑成功已保存')">
                    编辑
                  </a-button>
                  <a-button @click="$alert('删除成功')">删除</a-button>
                </a-space>
              </template> -->
              <a-descriptions-item :label="item.field1.key" :span="2">
                {{ item.field1.value }}
              </a-descriptions-item>
...
const data = Mock.mock({
  "data|9": [
    {
      "key|+1": 1,
      "title|1": ["节能措施", "节能措施", "节能措施", "节能措施"],
      bianhao() {
        return "JNCS072" + this.key;
      },
      field1: {
        "key|+1": "电能",
